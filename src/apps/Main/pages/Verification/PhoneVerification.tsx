...
type PhoneVerificationProps = object;
...
const PhoneVerification: FC<PhoneVerificationProps> = () => {
  ...
  const onSubmit: SubmitHandler<VerificationType> = useCallback(async (values) => {
    ...
    const success = await accountSecurityUseCase.verifyPhone(code);
    if (!success) {
      if (snackbarRef.current) closeSnackbar(snackbarRef.current);
  ...
  return (
    <div className='h-full flex items-center justify-center py-6'>
      <VerificationCard
        title={t('verification.phone.title')}
        description={t('verification.phone.description', { 
          name: profile?.givenName,
          phoneNumber: `${profile?.phoneNumberPrefix}${profile?.phoneNumber}`,
        })}>
          ...
          <Controller
            render={({ field, fieldState, }) => (
              <TextField 
                {...field}
                variant='outlined'
                label={t('verification.phone.label')} 
